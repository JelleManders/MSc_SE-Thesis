...
def run_all_2s():
	bs = Process(target=run_NR_P_comp_2s, args=('bs',))
	mc = Process(target=run_NR_P_comp_2s, args=('mc',))
	pa = Process(target=run_NR_P_comp_2s, args=('pa',))
	pr = Process(target=run_NR_P_comp_2s, args=('pr',))
	rb = Process(target=run_NR_P_comp_2s, args=('rb',))
	rh = Process(target=run_NR_P_comp_2s, args=('rh',))
	sh = Process(target=run_NR_P_comp_2s, args=('sh',))
	sf = Process(target=run_NR_P_comp_2s, args=('sf',))
	st = Process(target=run_NR_P_comp_2s, args=('st',))
	tr = Process(target=run_NR_P_comp_2s, args=('tr',))
	bs.start()
	mc.start()
	pa.start()
	pr.start()
...
def run_NR_P_comp_2s(category):
	log_file = open("Logs/NR-P_comp_2s_" + category + ".txt", "w")
	log_file.write(t() + " > Starting NR-P metrics of 2 second compressed videos..." + "\n")
	results_file = open("../resources/LiveVD/Scores/NR-P_comp_2s_" + category + "_scores.txt", "w")
	log_file.write(t() + " > Opened results file..." + "\n")
	for video in LiveVD_dis_cat_segLen(category, 2):
		try:
	...
	results_file.close()
	log_file.write(t() + " > Done. Results can be found in ../resources/LiveVD/Scores/NR-P_comp_2s_" + category + "_scores.txt" + "\n")
	log_file.close()
